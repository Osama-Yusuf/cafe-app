...
function MetricCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="bg-bg2 rounded-lg p-4 text-center">
      ...
      <div className="text-[0.6rem] text-text3 uppercase tracking-wider mt-1">{label}</div>
    </div>
  );
}
...
export function DashboardPage() {
  ...
  return (
    <div>
      <p className="text-xs text-gold font-semibold tracking-widest uppercase mb-1 flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-gold animate-pulse" />
        {cafeName} — Business Plan
      </p>
      <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-gold font-serif mb-1">
        {cafeName}
      </h1>
      <p className="text-text2 text-sm mb-8 max-w-lg">
        Your specialty café business planner. Every number is editable. Every calculation updates live.
      </p>

      {/* Metrics */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-border rounded-lg overflow-hidden mb-8">
      ...
      {/* Cost Pie Chart */}
      <h3 className="text-sm font-bold text-text2 mb-3">Where Your Money Goes (Monthly)</h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ background: '#181818', border: '1px solid #252525', borderRadius: 8, fontSize: 12 }}
                    itemStyle={{ color: '#ece5db' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
              ...
              {pieData.map((d) => {
                ...
                return (
                  <div key={d.name} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-text2">{d.name} ({pct}%)</span>
                    <span className="ml-auto text-gold font-semibold tabular-nums">{fmtK(d.value)}</span>
                  </div>
                );
              })}
            </div>
      ...
      {/* Market Context */}
      <h3 className="text-sm font-bold text-text2 mb-3">Egypt Market Context</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          { val: '2x', desc: 'Coffee consumption doubled in 5 years', color: '#d4a54a' },
          ...
          <Card key={m.val} className="bg-card border-border text-center p-4">
            <div className="text-2xl font-extrabold" style={{ color: m.color }}>{m.val}</div>
            <p className="text-xs text-text2 mt-1">{m.desc}</p>
          </Card>
        ))}
      </div>

      {/* Footer */}
      <div className="mt-10 pt-4 border-t border-border flex justify-between items-center text-text3">
        <span className="text-xs">
          Built by <a href="mailto:osama9mohamed5@gmail.com" className="text-gold hover:opacity-70 transition-opacity">Osama Yusuf</a>
        </span>
        <span className="text-[0.6rem] tracking-wide">&copy; 2026</span>
      </div>
    </div>
  );
}
